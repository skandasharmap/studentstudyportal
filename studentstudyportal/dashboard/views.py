        ...
        if 'items' in answer:
            for i in range(min(10, len(answer['items']))):
                ...
                result_dict = {
                    ...
                    'rating': item.get('averageRating'),  # Correct key
...
def dictionary(request):
    if request.method == "POST":
        ...
        try:
            ...
            for phonetic in answer[0].get('phonetics', []):
            ...
            # Loop through all definitions in all meanings
            for meaning in answer[0].get('meanings', []):
                for defn in meaning.get('definitions', []):
                    if defn.get('definition') and not definition:
                        definition = defn['definition']
                    if defn.get('example') and not example:
                        example = defn['example']
                    if 'synonyms' in defn:
                        synonyms.update(defn['synonyms'])
            ...
            if 'input' in request.POST:
                ...
                if input and int(input) >= 0:
                    ...
                    if first == 'kilogram' and second == 'pound':
                        answer = f'{input} kilogram={int(input) *2.20462} yard'
                context={
                    'form': form,
                    'm_form': measurement_form,
                    'input': True,
